...
impl SemanticIndex {
  ...
  pub async fn insert_belief_embeddings(&mut self, belief: &Belief) -> AppResult<()> {
    ...
    embedding_entries.push(self.create_embedding_entry(&belief.id, "content", &belief.content)?);
    ...
    for query in &belief.possible_queries {
      embedding_entries.push(self.create_embedding_entry(&belief.id, "possible_query", query)?);
    }
  ...
  pub async fn query_beliefs(
    ...
    let mut candidates = best_by_belief_id
      .into_keys()
      .map(|belief_id| {
